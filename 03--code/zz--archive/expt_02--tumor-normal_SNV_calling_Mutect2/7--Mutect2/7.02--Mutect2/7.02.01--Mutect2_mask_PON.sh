#!/bin/bash
#SBATCH --job-name mutect2MaskPon.sh 			### Job name
#SBATCH -o Output_Log-mutect2MaskPon.sh.log	### File to store output
#SBATCH -e Error_Log-mutect2MaskPon.sh.err	### File to store error messages
...
-O "$OUT_VCF_BAM_PREFIX".mutect2.maskPon.vcf \
-bamout "$OUT_VCF_BAM_PREFIX".mutect2.maskPon.reassem.bam \
-TMP_DIR=$TMPDIR
