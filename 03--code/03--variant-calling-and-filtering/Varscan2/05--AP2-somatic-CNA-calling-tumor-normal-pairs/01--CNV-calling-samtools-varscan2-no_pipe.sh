#!/bin/bash
#SBATCH --job-name=CNV-calling	                ### Job name
#SBATCH --output=Output_Log-CNV-calling.log	### File to store output
#SBATCH --error=Error_Log-CNV-calling.err	    ### File to store error messages
...
# 1) Make a tumor-normal .mpileup file and pipe to VarScan2 copynumber command
    ...
    # Run Varscan2 copynumber on the mpileup file
    java -jar $varscan_jar copynumber \
    "$out_basename".mpileup \
    "${results_dir_name}/${out_basename}" \
    --mpileup 1
    # Delete the large mpileup file
    rm "$out_basename".mpileup
...
# 3) Apply circular binary segmentation in R using the library DNAcopy from Bioconductor
    ...
    # Rscript 02--CBS-DNAcopy.r <copy_number_file> <output_basename> <results_dir_name>
    Rscript 02--CBS-DNAcopy.r "${results_dir_name}/${out_basename}.copynumber.called" "$out_basename" "$results_dir_name"
