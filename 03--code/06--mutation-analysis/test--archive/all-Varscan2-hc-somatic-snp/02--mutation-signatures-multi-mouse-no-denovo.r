...
# args = commandArgs(trailingOnly = T)
    ...
    setwd("04--analysis/expt_06--mutation-signature-analysis/multi/all-Varscan2-hc-somatic-snp/")
...
# 4) Mutational signatures ------------------------------------------------
    ...
    cosmicv3_sigs_no_arts = read.csv(file = "../../../../02--data/02--reference-data/COMIC-mutation-signatures/COSMIC-mutation-signatures-v3-no-artif-formatted.csv", header = T)
      # Old - for COSMICv2:
      ...
      sigs_pole = c("SBS10a", "SBS10b", "SBS28")
      sigs_mmr = c("SBS6", "SBS15", "SBS26", "SBS44")
      sigs_pole_mmr_combo = "SBS14"
      sigs_apobec = c("SBS1", "SBS2", "SBS13", "SBS84", "SBS85")
      relevant_signatures = c(sigs_pole, sigs_mmr, sigs_pole_mmr_combo, sigs_apobec)
    ...
    fit_res_contrib_mat_rev_df = data.frame(fit_res_contrib_mat_rev)
    fit_res_contrib_mat_rev_df = fit_res_contrib_mat_rev_df %>% 
      rownames_to_column("COSMIC_signature")
    write_tsv(x = fit_res_contrib_mat_rev_df, path = paste0(results_tables_dir, "03--COSMIC-contrib-matrix.txt"), )
    
# Define custom color palette for main sigs present (some will be missing bc not in matrix)
# POLE sigs (10a, 10b, 28)
sigs_pole_in_contrib = sigs_pole[sigs_pole %in% rev_order_contrib_rownames]
# Make appropriate length color palette
if (length(sigs_pole_in_contrib) == 1) {
  col_pole = rev(brewer.pal(5, "Blues"))[1]
} else if (length(sigs_pole_in_contrib) == 2) {
  col_pole = rev(brewer.pal(5, "Blues"))[1:2]
} else if (length(sigs_pole_in_contrib) >= 3) {
  col_pole = rev(brewer.pal(5, "Blues"))[1:3]
} else {
  col_pole = NA
}

# MMR sigs (6, 15, 26, 44)
sigs_mmr_in_contrib = sigs_mmr[sigs_mmr %in% rev_order_contrib_rownames]
# Make appropriate length color palette
if (length(sigs_mmr_in_contrib) == 1) {
  col_mmr = rev(brewer.pal(5, "Reds"))[1]
} else if (length(sigs_mmr_in_contrib) == 2) {
  col_mmr = rev(brewer.pal(5, "Reds"))[1:2]
} else if (length(sigs_mmr_in_contrib) == 3) {
  col_mmr = rev(brewer.pal(5, "Reds"))[1:3]
} else if (length(sigs_mmr_in_contrib) == 4) {
  col_mmr = rev(brewer.pal(5, "Reds"))[1:4]
} else {
  col_mmr = NA
}

# Combo sigs (14)
sigs_combo_in_contrib = sigs_pole_mmr_combo[sigs_pole_mmr_combo %in% rev_order_contrib_rownames]
# Make appropriate length color palette
if (length(sigs_combo_in_contrib) == 1) {
  col_combo = rev(brewer.pal(5, "Purples"))[1]
} else {
  col_combo = NA
}

# APOBEC sigs (1, 2, 13, 84, 85)
sigs_apobec_in_contrib = sigs_apobec[sigs_apobec %in% rev_order_contrib_rownames]
# Make appropriate length color palette
if (length(sigs_apobec_in_contrib) == 1) {
  col_apobec = rev(brewer.pal(5, "Oranges"))[1]
} else if (length(sigs_apobec_in_contrib) == 2) {
  col_apobec = rev(brewer.pal(5, "Oranges"))[1:2]
} else if (length(sigs_apobec_in_contrib) == 3) {
  col_apobec = rev(brewer.pal(5, "Oranges"))[1:3]
} else if (length(sigs_apobec_in_contrib) == 4) {
  col_apobec = rev(brewer.pal(5, "Oranges"))[1:4]
} else if (length(sigs_apobec_in_contrib) == 5) {
  col_apobec = rev(brewer.pal(5, "Oranges"))[1:5]
} else {
  col_apobec = NA
}

# All contributing relevant signatures
all_rel_sigs_in_contrib = c(sigs_pole_in_contrib, sigs_mmr_in_contrib, sigs_combo_in_contrib, sigs_apobec_in_contrib)
# Number of contributing relevant signatures
num_rel_contrib_sigs = length(all_rel_sigs_in_contrib)
# Number of non-relevant signatures
num_nonrel_contrib_sigs = length(rownames(fit_res_contrib_mat_rev)) - length(all_rel_sigs_in_contrib)
        
# Colors for other present signatures
# cols_ramp = rev(c("#ffffff", "#dddddd", "#ffdddd", "#ddffdd", "#ddddff", "#ffffdd", "#ffddff", "#ddffff"))
col_others = colorRampPalette(brewer.pal(5, "Greys"))(num_nonrel_contrib_sigs)
# Total custom color palette of 30 colors
color_palette_combined = c(col_others, col_apobec, col_combo, col_mmr, col_pole)
color_palette_combined = color_palette_combined[!is.na(color_palette_combined)]
...
# 6) Genomic distribution -------------------------------------------------
    ...
    pdf(file = paste0(results_plots_dir, "14--genomic-rainfall-all.pdf"), width = 16, height = 9)
    grid.arrange(rainfall_m079, rainfall_m084, rainfall_m122, rainfall_m124, rainfall_m1098)
    dev.off()
